...
def add_transaction():
    if request.method == 'POST':
        if 'search_transaction_id' in request.form:
...
def aggregates():
    highest_total_price = collection_transactions.aggregate([
        {"$sort": {"total_price": -1}}
    ])

    lowest_total_price = collection_transactions.aggregate([
        {"$sort": {"total_price": 1}}
    ])

    sort_by_date = collection_transactions.aggregate([
        {"$sort": {"time": 1}}
    ])

    sort_by_share_name = collection_transactions.aggregate([
        {"$sort": {"Share_name": 1}}
    ])

    sort_name_AZ = collection_transactions.aggregate([
        {"$lookup": {
            "from": "users",
            "localField": "stocknumber_id",
            "foreignField": "stocknumber_id",
            "as": "user_info"
        }},
        {"$unwind": "$user_info"},
        {"$sort": {"user_info.fullname": 1}}
    ])

    sort_name_ZA = collection_transactions.aggregate([
        {"$lookup": {
            "from": "users",
            "localField": "stocknumber_id",
            "foreignField": "stocknumber_id",
    ...
    return render_template("aggregates.html", 
                           ...
                           sort_by_share_name=sort_by_share_name,
